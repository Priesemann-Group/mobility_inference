...
shutil.copyfile("model.py", f"{supdir_name}/code_for_{name}.py")
shutil.copyfile("utils.py", f"{supdir_name}/utils_for_{name}.py")
shutil.copyfile("plot_all.py", f"{supdir_name}/plotting_for_{name}.py")
...
average_tmax = xr.DataArray(
    average_tmax,
    dims="date",
    coords={"date": mobility_dates_2020},
    name="average_tmax",
)


# Disease spread indicators
...
for indicators in all_combinations:
    ...
    # for saving the individual results
    tag = ""
    ...
    # Inference
    trace = pm.sample(model=model, draws=200, tune=200, cores=1, chains=2)
    with model:
    ...
    plot.analysis_figures(model, trace, tag, subFigDir_name, dates, indicators)


# To Do: Plot model selection figures
