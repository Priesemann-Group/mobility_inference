...
def create_model(
    ...
    with model_in:
        ...
        for indicator in indicators_in:
            ...
            mu_disease = pm.LogNormal(f"mu_{indicator}", mu=np.log(1.0), tau=5)
            sigma_disease = pm.LogNormal(f"sigma_{indicator}", mu=np.log(1.0), tau=5)
            ## convolution
        ...
        # impact of weather: precipitation
...
""" Model with temperature and no pandemic fatigue
