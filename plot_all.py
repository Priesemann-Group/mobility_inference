...
def create_figure_dir(dir_name_in, tag_in):
    # We create a directory for the results.
    dir_name = dir_name_in + "/" + tag_in
...
def plot_gamma_kernel(trace_in, dir_name_in, indicators_in):
...
def plot_temperature_timeseries(dates_in, trace_in, tag_in):
    ...
    # save figure
    fig.savefig(f"{dir_name_in}/temperature.png", bbox_inches="tight")
    fig.savefig(f"{dir_name_in}/temperature.pdf", bbox_inches="tight")


## plot ood time series
def plot_out_of_home_duration_timeseries(
    dates_in, trace_in, dir_name_in, indicators_in
):
    ...
    # save figure
    fig.savefig(f"{dir_name_in}/out_of_home_duration.png", bbox_inches="tight")
    fig.savefig(f"{dir_name_in}/out_of_home_duration.pdf", bbox_inches="tight")


# plot distribution for single indicator models
def plot_distributions(model_in, trace_in, dir_name_in, indicators_in):
    ...
    cov19.plot.distribution(model_in, trace_in, "z_S", dist_math="z_{S}", ax=axs[6])
    ...
    fig.savefig(f"{dir_name_in}/distributions.png", dpi=300, bbox_inches="tight")
    fig.savefig(f"{dir_name_in}/distributions.pdf", dpi=300, bbox_inches="tight")


def analysis_figures(model_in, trace_in, tag_in, dir_name_in, dates_in, indicators_in):
    dir_name = create_figure_dir(dir_name_in, tag_in)

    plot_distributions(model_in, trace_in, dir_name, indicators_in)
    plot_temperature_timeseries(dates_in, dir_name, tag_in)
    plot_gamma_kernel(trace_in, dir_name, indicators_in)
    plot_out_of_home_duration_timeseries(dates_in, trace_in, dir_name, indicators_in)
